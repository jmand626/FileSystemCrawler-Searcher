...
static void HandleDir(char* dir_path, DIR* d, DocTable** doc_table,
  ...
  int i = 0;
  // Changed in HW2, since we are using while loop
  int path_name_len;
  ...
  dirent = readdir(d);
  // We do it like this because we need to assign dirent in 3 different stops:
  // Before loop, after first if statement, and at end of loop
  while (dirent != NULL) {
    // STEP 2.
    ...
    if ((strcmp(dirent->d_name, ".") == 0) ||
        ...
        // strcmp is a direct, exact compare of strings
     dirent = readdir(d);
     continue;
    }
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      // This "is_dir" field is used in the next for loop in the second pass
      // Therefore, we have to use it here
      // The isreg and isdir macros are self-explanatory
      if (S_ISDIR(st.st_mode)) {
        ...
        continue;
        // Skip if something else scary
      }
    }

    dirent = readdir(d);
    i++;
    // This i++ line was added as we now use a while loop instead (in step 1)
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  // of the file.
  // So obviously we need to call that function, but that function is defined
  // with an argument called file_contents, so we do actually need to call
  // ReadfiletoString first
  contents = ReadFileToString(file_path, &file_len);
  tab = ParseIntoWordPositionsTable(contents);

  // Quick error check / get out of this function check
  if (tab == NULL) {
    return;
  }
  ...
  while (HTIterator_IsValid(it)) {
    ...
    // remember to remove an iterm in a middle of a list, we must
    // use an iterator
    wp = (WordPositions*)kv.value;
    MemIndex_AddPostingList(*index, wp->word, doc_id, wp->positions);
    // Since the wp hashtable has a linkedlist, we -> to get it here
