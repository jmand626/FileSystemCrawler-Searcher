...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  // As explained in the bash file, all of this family of system calls return
  // stat structures through output parameters, and an integer where 0 is
  // success and -1 is error. We send in the & of the struct stat as the docs
  // say
  ...
  if (!S_ISREG(file_stat.st_mode)) {
    return NULL;
    // Actually in man 7 inode, we can use ISREG and the .st_mode field,
    // where this field contains the file type and mode
  }
  ...
  // Char * cast at end to use it for file reading, +1 for null terminator at
  // end of buffer optional to verify malloc integrity in this class
  ...
  // num_read is how much we read in case read stops for some reason min read
  left_to_read = file_stat.st_size;
  while (left_to_read > 0) {
    result = read(fd, buf + (num_read-left_to_read), left_to_read);
    // Now either success, bad error->quit, or recoverable error

    if (result == -1 && (errno != EINTR && errno != EAGAIN)) {
      close(fd);
      free(buf);
      // Must remember to always free memory even when failing
      perror("Unrecoverable error with read");
      return NULL;
    } else if (result == 0) {
      // success, get out of loop naturally
      left_to_read = 0;
    }
    left_to_read -= result;
  }
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  // *cur_ptr = tolower(*cur_ptr)).  Finally, as a hint, you can overwrite
  ...
  // C's equivalent of a while(true) loop
  while (*cur_ptr != '\0') {
    if (isalpha(*cur_ptr)) {
        // Valid alphabetic character, part of a word
        // Convert to lowercase
        *cur_ptr = tolower(*cur_ptr);

        // If this is the first letter of a new word
        // cur_ptr == word_start || *(cur_ptr - 1) == '\0')
    } else {
        // Non-alphabetic character (word boundary), terminate the word
        *cur_ptr = '\0';

        // Add the word to the hash table if it hasn't already been added
        if (cur_ptr != word_start) {
            AddWordPosition(tab, word_start, word_start - content);
        }

        // Set word_start to the next possible word start
  ...
  // Add the final word (if any) after the loop
  if (cur_ptr != word_start) {
    // Because now, cur_ptr would be at the end of the word
      AddWordPosition(tab, word_start, word_start - content);
  }
}
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // optional to verify malloc integrity in this class

    char* string = (char*)malloc(strlen(word)+ 1);
    // Fill out our new structure but copying over letters from word to string
    ...
    // "using a similar ugly hack"
    LinkedList_Append(wp->positions, (LLPayload_t) (int64_t) pos);


    kv.key = hash_key;
    kv.value = (void*)wp;
    HTKeyValue_t toReturn;


    HashTable_Insert(tab, kv, &toReturn);
  }
}
